...
test("server-renders the branded CampusOne bootstrap and landing source",async()=>{
  ...
  const landing=await readFile(new URL("../app/Landing.tsx",import.meta.url),"utf8");
...
test("ships protected APIs, role policies and durable storage configuration",async()=>{
  const [campus,auth,google,callback,upload,hosting,page,schema]=await Promise.all([
  ...
  for(const moduleName of ["Attendance","Assignments","Events","Placements","Clubs","Calendar","Messages","Settings","Admin"]) assert.match(page,new RegExp(moduleName));
});
